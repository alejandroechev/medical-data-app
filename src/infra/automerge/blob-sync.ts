...
const BLOB_DB_NAME = "medapp-blobs";
...
/**
 * Store a file locally and queue it for server upload.
...
export async function storeAndSyncBlob(file: File): Promise<{ blobId: string; localUrl: string; sizeBytes: number }> {
  ...
  // Try uploading now (non-blocking)
...
/**
 * Get a blob — local first, fetch from server if missing.
 */
export async function getBlobUrl(blobId: string): Promise<string | null> {
  ...
  try {
    ...
    await storeLocalBlob(blobId, data); // cache locally
...
/**
 * Retry all pending uploads. Call this on reconnect.
 */
export async function retryPendingUploads(): Promise<{ total: number; succeeded: number }> {
  ...
  for (const blobId of pending) {
    ...
    if (!data) {
      await removeFromPendingQueue(blobId); // orphan entry
      continue;
    }
...
/**
 * Get count of pending uploads (for UI indicator).
 */
...
function notifyBlobSynced(blobId: string) {
  if (typeof window !== "undefined") {
    window.dispatchEvent(new CustomEvent("medapp:blob-synced", { detail: { blobId } }));
  }
}
