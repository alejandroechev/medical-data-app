...
describe('PhotoLinker', () => {
  ...
  it('should render the link photo button', () => {
    ...
    expect(screen.getByRole('button', { name: /agregar documento/i })).toBeInTheDocument();
  });
  ...
  it('should show the form when Pegar URL is clicked', async () => {
    ...
    expect(screen.getByLabelText(/url del documento/i)).toBeInTheDocument();
  ...
  it('should call onPhotoLinked with correct data when form is valid', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /agregar documento/i }));
    await user.click(screen.getByText('Pegar URL'));
    await user.type(screen.getByLabelText(/url del documento/i), 'https://example.com/photo/abc123');
  ...
  it('should hide the form after successful link', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /agregar documento/i }));
    await user.click(screen.getByText('Pegar URL'));
    await user.type(screen.getByLabelText(/url del documento/i), 'https://example.com/photo/abc');
    await user.click(screen.getByRole('button', { name: /guardar/i }));

    await vi.waitFor(() => {
      expect(screen.queryByLabelText(/url del documento/i)).not.toBeInTheDocument();
    });
  });
